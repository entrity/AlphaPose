...
class Example(object):
	...
	def mask(self, binmask):
		...
		binmask = np.expand_dims(binmask, 2)
		imdata *= binmask
		if opts.spread:
			offsets = [ v for v in range(opts.spread) ]
			for offset in offsets:
				if offset:
					imdata[offset:,:,:] *= binmask[:-offset,:,:] # down
					imdata[:,offset:,:] *= binmask[:,:-offset,:] # right
					imdata[:-offset,:,:] *= binmask[offset:,:,:] # up
					imdata[:,:-offset,:] *= binmask[:,offset:,:] # left
		return imdata
